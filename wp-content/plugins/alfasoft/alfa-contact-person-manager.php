...
function enqueue_admin_scripts() {
    ...
    wp_localize_script('my-admin-js', 'myAdmin', array(
...
function add_person_contact_menu() {
    add_menu_page('Alfasoft Manager', 'Alfasoft Manager', 'manage_options', 'alfasoft-manager', 'render_person_contact_manager_page');
}
...
function render_edit_person_page() {
    ...
    if (isset($_GET['id'])) {
        ...
        $person = $wpdb->get_row($wpdb->prepare("SELECT * FROM $people_table WHERE id = %d", $id));

        // Check if form is submitted
        if (isset($_POST['edit_person'])) {
            $name = sanitize_text_field($_POST['name']);
            $email = sanitize_email($_POST['email']);
            $existing_person = $wpdb->get_var($wpdb->prepare("SELECT id FROM $people_table WHERE email = %s", $email));
			...
			echo '<a href="' . esc_url(home_url('/wp-admin/admin.php?page=manage-people')) . '">Go Back</a>';
        ...
        echo '<input type="hidden" name="id" value="' . esc_attr($person->id) . '">';
  ...
  function render_edit_contact_page() {
    ...
    if (isset($_GET['id'])) {
        $id = intval($_GET['id']); // Sanitize the ID to prevent SQL injection
        $contact = $wpdb->get_row($wpdb->prepare("SELECT * FROM $contacts_table WHERE id = %d", $id));

        // Check if form is submitted
        if (isset($_POST['edit_contact'])) {
            $country_code = sanitize_text_field($_POST['country_code']);
            $number = sanitize_text_field($_POST['number']);
            $existing_contact = $wpdb->get_var($wpdb->prepare("SELECT id FROM $contacts_table WHERE country_code = %s AND number = %s AND id != %d", $country_code, $number, $id));
            ...
            // Update the contact
            $wpdb->update($contacts_table, array(
        ...
        echo '<input type="text" id="number" name="number" value="' . esc_attr($contact->number) . '" required><br>';
...
function enqueue_admin_styles() {
    wp_enqueue_style('my-admin-css', plugin_dir_url(__FILE__) . 'css/admin-styles.css');
}
...
function render_manage_people_page() {
    ...
    if (isset($_POST['add_person'])) {
        ...
        $wpdb->insert($people_table, array(
            'name' => $name,
            'email' => $email
        ));
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'delete') {
        $id = intval($_GET['id']);
        $wpdb->update($people_table, array('deleted' => 1), array('id' => $id));
	...
	$people = $wpdb->get_results("SELECT * FROM $people_table WHERE deleted = 0");
    echo '<div class="wrap">';
    echo '<h2>People</h2>';
    ...
    foreach ($people as $person) {
        ...
        echo '<td><a href="?page=edit-person&id=' . $person->id . '">' . esc_html($person->name) . '</a></td>';
        echo '<td>' . esc_html($person->email) . '</td>';
        echo '<td>';
        echo '<a href="?page=add-contact&person_id=' . $person->id . '">Add Contact</a> | ';
        echo '<a href="?page=edit-person&id=' . $person->id . '">Edit</a> | ';
        echo '<a href="?page=manage-people&action=delete&id=' . $person->id . '">Delete</a>';
        echo '</td>';
        echo '</tr>';
    }
    echo '</table>';
...
function render_manage_contact_page() {
    ...
    if (isset($_POST['add_contact'])) {
        ...
        $number = sanitize_text_field($_POST['number']);
        $existing_contact = $wpdb->get_var($wpdb->prepare("SELECT id FROM $contacts_table WHERE country_code = %s AND number = %s", $countryCode, $number));
        ...
        if ($person_id !== null) {
            $person_exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $people_table WHERE id = %d", $person_id));
    ...
    foreach ($countries as $country) {
        ...
        echo '<option value="' . esc_attr($callingCode) . '">' . $country['name']['common'] . ' (' . esc_html($callingCode) . ')</option>';
    }
    echo '</select>';
    echo '<label for="number">Number:</label>';
...
if ($person_id !== null) {
    $contacts = $wpdb->get_results($wpdb->prepare("SELECT * FROM $contacts_table WHERE person_id = %d", $person_id));
} else {
    // Handle the case where no person_id is provided
    echo '<div id="message" class="error notice is-dismissible"><p>No person ID provided.</p></div>';
    return;
}
$contacts = $wpdb->get_results($wpdb->prepare("SELECT * FROM $contacts_table WHERE person_id = %d AND deleted = 0", $person_id));
...
echo '<a class="delete-contact-button" href="?page=add-contact&action=soft_delete_contact&person_id=' . $person_id . '&contact_id=' . $contact->id . '" data-contact-id="' . $contact->id . '">Delete</a>';
    echo '</td>';
    echo '</tr>';
}
echo '</tbody>';
...
if (isset($_GET['action']) && $_GET['action'] == 'soft_delete_contact' && isset($_GET['contact_id'])) {
    $contact_id = intval($_GET['contact_id']);
    $result = $wpdb->update($contacts_table, array('deleted' => 1), array('id' => $contact_id));
...
function alfasoft_manager_list_shortcode() {
    ...
    if ($results) {
        foreach ($results as $result) {
            ...
            $output .= '<td>' . esc_html($result->country_code) . ' ' . esc_html($result->number) . '</td>';
            $output .= '</tr>';
        }
    } else {
